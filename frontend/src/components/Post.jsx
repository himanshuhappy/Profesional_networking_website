...
function Post({id,author,like,comment,description,image,createdAt}) {
    ...
    //for realtime like update
    //taking input from backend(on-> listning from likeUpdate)
    useEffect(()=>{
    ...
    //for auto update of likes
    useEffect(()=>{
        getPost()
    },[likes,comments]) //depends on likes and setlikes
  return (
    <div className="w-full min-h-[200px] flex flex-col gap-[10px] bg-white rounded-lg shadow-lg p-[20px]">
       <div className='flex justify-between items-center'>
            ...
            <div className='flex justify-center items-start gap-[10px]'>
                ...
                {/* to use moment function install moment- npm install moment*/}
                <div className='text-[16px]'>{moment(createdAt).fromNow()}</div>
                </div>
            </div>
            <div>
                    {/* adding connection button */}
                    {/* we will only show connect button when reciever is not user */}
        ...
        <div>
            <div className='w-full flex justify-between items-center p-[20px] border-b-2 border-gray-500'>
                ...
                <BiLike className=' text-blue-500 w-[24px] h-[24px]'/>
            ...
            {showComment && <div>
                <form className="w-full flex justify-between items-center border-2 border-b-gray-500 p-10px" onSubmit={handleComment}>
